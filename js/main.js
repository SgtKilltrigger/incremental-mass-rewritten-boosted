...
const FORMS = {
    ...
    bh: {
        ...
        DM_gain() {
            ...
            gain = gain.root(4)
            if (player.ranks.tetr.gte(1)) gain = gain.mul(player.ranks.tetr.add(1)).max(1)
            if ((player.supernova.times.gte(1)) && (player.inf.theorem.lt(1)) && player.ranks.tetr.gte(1)) gain = gain.mul(player.ranks.tetr.pow(100).max(1).pow(player.supernova.times.add(1).max(1)))
